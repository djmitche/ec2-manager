...
const {SpotRequestPoller} = require('./spot-request-poller');
...
let load = loader({
  ...
  knex: {
    ...
    setup: async ({cfg}) => {
      ...
      return knex({
        ...
        pool: {
          min: cfg.postgres.minClients || 1, // TOOD: 4
  ...
  state: {
    requires: ['knex', 'cfg', 'monitor'],
    setup: async ({knex, cfg, monitor}) => {
      return new State({knex, databaseUrl: cfg.postgres.databaseUrl, monitor});
    },
  },
  ...
  eventlisteners: {
    requires: ['cfg', 'sqs', 'ec2', 'profile', 'monitor', 'state', 'tagger'],
    setup: async ({cfg, sqs, ec2, profile, monitor, state, tagger}) => {
  ...
  spotrequestpoller: {
    ...
    setup: async ({cfg, state, ec2, monitor, runaws}) =>
      new SpotRequestPoller({
        ec2,
        regions: cfg.app.regions,
        runaws,
        spotPollDelay: cfg.app.spotPollDelay,
        state,
      }),
  },

  housekeeper: {
    requires: ['cfg', 'state', 'ec2', 'monitor', 'runaws', 'tagger'],
    setup: async ({cfg, state, ec2, monitor, runaws, tagger}) => {
  ...
  start: {
    requires: ['eventlisteners', 'deadeventlisteners', 'server', 'spotrequestpoller', 'housekeeper', 'pricing'],
    setup: async ({eventlisteners, deadeventlisteners, server, spotrequestpoller, housekeeper, pricing}) => {
      eventlisteners.map(x => x.start());
      deadeventlisteners.map(x => x.start());
      spotrequestpoller.start();
      housekeeper.start();
      pricing.start();
      log.info('Started');
    },
