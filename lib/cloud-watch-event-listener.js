...
class CloudWatchEventListener extends events.EventEmitter {
  ...
  async __handler(msg) {
    ...
    const instanceLogInfo = (instance, metadataSource) => ({
    ...
    await this.state.modifyInstance({
      region,
      id,
      modify: async ({instance, removeSpotRequest, upsertInstance, removeInstance}) => {
        if (instance) {
          ...
          if (instance.lastevent && instance.lastevent < generated) {
            try {
              if (state === 'pending' || state === 'running') {
                await upsertInstance(_.defaults({state, lastevent: generated}, instance));
                log.info(instanceLogInfo(instance, 'db'), 'CloudWatch Event resulting in upsertion');
              } else {
                await removeInstance();
                log.debug(instanceLogInfo(instance, 'db'), 'CloudWatch Event resulting in deletion');
              }
            } catch (err) {
            ...
            let delay = instance.lastevent - generated;
          ...
          if (workerType && provisionerId === this.provisionerId) {
            await removeSpotRequest({srid, region});
            await upsertInstance(newInstance);
            log.info(instanceLogInfo(newInstance, 'api'),
              'CloudWatch Event resulting in insertion');
          } else {
            log.debug(instanceLogInfo(newInstance, 'api'),
              'Ignoring newInstance because it does not belong to this manager');
          }
        } else {
          ...
          // we could do it here (and also in the instance truthy clause above).
          await removeInstance();
